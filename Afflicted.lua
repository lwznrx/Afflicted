...
function Afflicted:OnInitialize()
	self.defaults = {
		profile = {
			...
			spellVersion = 0,
			inside = {["arena"] = true, ["pvp"] = true, ["party"] = true, ["raid"] = true, ["none"] = true},
			anchors = {
				interrupts = {
					enabled = true,
					announce = false,
	...
	self.defaults.profile.anchors.interrupts = CopyTable(self.defaults.profile.anchors.interrupts)
	self.defaults.profile.anchors.interrupts.text = "Interrupts"
	self.defaults.profile.anchors.cooldowns = CopyTable(self.defaults.profile.anchors.cooldowns)
	self.defaults.profile.anchors.cooldowns.text = "Cooldowns"
	self.defaults.profile.anchors.spells = CopyTable(self.defaults.profile.anchors.spells)
	self.defaults.profile.anchors.spells.text = "Spells"
	self.defaults.profile.anchors.buffs = CopyTable(self.defaults.profile.anchors.buffs)
	self.defaults.profile.anchors.buffs.text = "Buffs"
	self.defaults.profile.anchors.defenses = CopyTable(self.defaults.profile.anchors.defenses)
	self.defaults.profile.anchors.defenses.text = "Defensive"
	self.defaults.profile.anchors.damage = CopyTable(self.defaults.profile.anchors.damage)
	self.defaults.profile.anchors.damage.text = "DR"
